...
- **Init flags** — new task from an existing reference file:
  ...
  [--no-code-checker]
  ```
  `--devices` selects the local Ascend NPU id(s) for eval; arch is
  auto-derived from the picked card via `npu-smi`.
...
- **Desc mode** — new task from a natural-language description:
  ```
  --desc "fused ReLU + LayerNorm, (32,1024), fp16" --devices 0
  ```
...
```json
{
  ...
  "values": {ref, desc, op_name, dsl, devices, max_rounds, ...},
  "missing": [...]
}
```
