...
class BuildExt(build_ext):
    def run(self):
        so_files = glob.glob(str(BUILD_DIR / "rms_norm_ext*.so"))
        if not so_files:
...
setuptools.setup(
    name="rms_norm",
    version="0.1.0",
    description="RMSNorm AscendC kernel — registers torch.ops.npu.rms_norm",
    ext_modules=[NpuExtension("rms_norm_ext", sources=[])],
